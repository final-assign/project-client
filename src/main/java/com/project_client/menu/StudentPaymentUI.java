...
import static StudentMenuInquiryUI.StaffMenuInquiryUI;
import static com.project_client.Main.sc;

public class StaffPaymentUI {

    public static void StudentPaymentUI(DataInputStream dis, DataOutputStream dos, Object selectedMenu) {
        try {
            // ===== 결제방법 선택 =====
            ...
            while (paymentChoice) {
                ...
                if (choice == 1) {
                    // 카드 결제 처리
                    boolean cardPaymentSuccess = processCardPayment(dis, dos, selectedMenu);

                    if (cardPaymentSuccess) {
                        System.out.println("카드 결제가 성공했습니다.");
                        paymentChoice = false;
                        // StudentMenuInquiryUI의 다음 행동 메뉴 호출
                        showNextActionMenu(dis, dos);
                        return;
                    } else {
                    ...
                    // 쿠폰 결제 처리
                    boolean couponPaymentSuccess = processCouponPayment(dis, dos, selectedMenu);
    ...
    private static boolean processCardPayment(DataInputStream dis, DataOutputStream dos, Object selectedMenu) {
        try {
    ...
    private static boolean processCouponPayment(DataInputStream dis, DataOutputStream dos, Object selectedMenu) {
        try {
